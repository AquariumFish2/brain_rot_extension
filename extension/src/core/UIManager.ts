...
export class UIManager {
  ...
  private setupEvents(): void {
    ...
    this.overlay.querySelectorAll('.theme-option').forEach(btn => {
      btn.addEventListener('click', () => {
        const key = (btn as HTMLElement).dataset.palette;
        if (key) ThemeManager.getInstance().applyPalette(key);
      });
    });

    // Listen for custom game events dispatched by MemoryGame.ts
    document.addEventListener('tf_game_win', (e: any) => this.showWinOverlay(e.detail));
    document.addEventListener('tf_game_lose', (e: any) => this.showLoseOverlay(e.detail));
  ...
  public renderLanding(): void {
    ...
    (['easy', 'normal', 'hard'] as Difficulty[]).forEach(diff => {
      ...
      if (score) {
        ...
        if (badge) badge.textContent = `Best: ${score}`;
      } else {
        box?.classList.remove('has');
        badge?.classList.remove('has');
        if (val) val.textContent = '—';
        if (badge) badge.textContent = 'No record';
      }
    });
  }
  ...
  private renderStreak(): void {
    ...
    if (countEl) countEl.textContent = String(data.streak);
    if (!daysEl) return;
   ...
   */
  private showWinOverlay(detail: any): void {
    ...
    if (scoreVal) scoreVal.textContent = detail.score;
    if (movesVal) movesVal.textContent = detail.moves;
    if (movesValS) movesValS.textContent = detail.moves;
    
    const m = Math.floor(detail.timeLeft / 60);
    const s = detail.timeLeft % 60;
    const ts = `${m}:${s < 10 ? '0' : ''}${s}`;
    if (timeVal) timeVal.textContent = ts;
    if (timeValS) timeValS.textContent = ts;
   ...
   */
  private showLoseOverlay(detail: any): void {
    const loseOverlay = document.getElementById('tf-loseOverlay');
    if (loseOverlay) loseOverlay.classList.add('on');
    ...
    if (pairsVal) pairsVal.textContent = `${detail.matched}/${detail.total}`;
    if (movesVal) movesVal.textContent = detail.moves;
    if (titleEl) titleEl.textContent = detail.skipped ? 'Skipped.' : "Time's up.";
    if (msgEl) msgEl.textContent = detail.skipped 
